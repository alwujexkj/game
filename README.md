...
家庭竖屏 H5：数学馆 · 英语岛 · 组队房间码 · 造关卡工坊 · **背包互赠 / 家庭排行 / 家长总览（M4）**。
...
### M4 背包 · 排行 · 家长
- 地图热点 **背包** → `/bag`：贴纸库存（gift-stickers 图集）、送给兄弟姐妹或「喂果冻」看反应
- 地图热点 **排行** → `/rank`：周星 / 最佳连击 / 组队次数，温柔文案、无嘲讽
- **家长** `/parent`：首次设置 4 位 PIN（本地哈希），解锁后看三孩周活动、通关星、轻轻关注的弱项标签（无完整错题 dump）
- 关卡结算有小概率掉落贴纸进背包
- 本地 localStorage + 可选 Nest `/api/gifts`、`/api/parent`（DB 不可用时降级）
...
试玩：选角色，进地图数学馆 / 英语岛 / 组队 / 工坊 / 背包 / 排行；家长入口设 PIN。
